...
draftsFolderName = '../_drafts/'

def ensure_drafts_folder():
  if not path.exists(draftsFolderName):
    os.makedirs(draftsFolderName)

def build_fileName(time, title):
...
for item in items:
  ...
  post_body = build_post_body(item)

  (season, episode) = parse_season_episode(item, title)

  f = open(draftsFolderName + fileName,'w') 
  f.write('---\ntype: post\nlayout: post\ntitle: ' + title + '\n')
